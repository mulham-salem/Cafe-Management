...
const CustomerNotification = () => {
  ...
  useEffect(() => {
     ...
     const fetchNotifications = async () => {
      try {
        const response = await axios.get('http://localhost:8000/api/user/customer/notifications', {
          headers: {
  ...
  const markAsSeen = async (id) => {
    try {
      await axios.patch(`http://localhost:8000/api/user/customer/notifications/${id}/seen`, {}, {
        headers: {
          Authorization: `Bearer ${token}`,
          Accept: 'application/json',
        },
